...
export default function App() {
  ...
  // Filter results by veg toggle ΓÇö this was MISSING causing ReferenceError crash
                         ...
                         {displayedResults.map((group, idx) => (
                             <TouchableOpacity key={idx} style={styles.restCard} onPress={() => setSelectedRest(group)}>
                                 ...
                                 <View style={styles.openMenuBtn}>
                                     <Text style={styles.openMenuText}>Open Menu Γ₧ö</Text>
                                 </View>
                             </TouchableOpacity>
                         ))}
                     </ScrollView>
          ...
          {activeTab === 'Search' && activeCategory === 'Food' && selectedRest && (
             <View style={{flex: 1}}>
                 <TouchableOpacity style={styles.backBtn} onPress={() => setSelectedRest(null)}>
                     <Text style={{fontSize: 16}}>ΓåÉ Back to {searchQuery ? 'search' : 'restaurants'}</Text>
                 </TouchableOpacity>
                         ...
                         <View key={dIdx} style={styles.dishCard}>
                             ...
                             {dish.offers.map((offer: any, oIdx: number) => (
                                 <View key={oIdx} style={styles.offerRow}>
                                     ...
                                     <Text style={styles.offerPrice}>Γé╣{offer.price?.finalPayablePrice || offer.price?.basePrice || 0}</Text>
                                 </View>
                             ))}
                             <TouchableOpacity 
                                style={styles.addBtn}
