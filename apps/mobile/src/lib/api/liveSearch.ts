...
export async function fetchLiveSwiggyDishes(
    ...
    try {
        ...
        cardsList.forEach((c: any) => {
            ...
            if (info && info.name) {
                ...
                if (price > 0) {
                    ...
                    let allDiscountText = `${discountHeader} ${discountSubHeader} ${descMeta}`.trim();
                    ...
                    if (percentMatch) {
                        couponPercent = parseInt(percentMatch[1], 10);
                    }
                    ...
                    let autoSavings = 0;
                    if (couponPercent > 0) {
                        autoSavings = Math.round((finalPrice * couponPercent) / 100);
                        if (couponMaxCap > 0 && autoSavings > couponMaxCap) {
                            autoSavings = couponMaxCap;
                        }
                    }
                    ...
                    if (flatMatch && !allDiscountText.includes('%')) {
                        ...
                        if (couponFlat > 0 && autoSavings === 0) {
                            autoSavings = couponFlat;
                        }
                    }

                    const effectivePrice = Math.max(0, finalPrice - autoSavings);

                    const additionalOffers: any[] = [];
                    const otherOffers = restInfo?.aggregatedDiscountInfoV2?.descriptionList || [];
                    otherOffers.forEach((o: any) => {
                        if (o.meta && o.meta !== discountHeader) {
                            additionalOffers.push({
                                title: o.meta,
                                code: o.meta.match(/(?:USE\s+CODE|CODE)\s+([A-Z0-9]+)/i)?.[1] || '',
                                description: o.meta
                            });
                        }
                    });
                    ...
                    items.push({
                        ...
                        restaurantUrl: `https://www.swiggy.com/restaurants/${restInfo?.slugs?.restaurant || ''}-${restInfo?.id || ''}`,
                        menuPrice: finalPrice,
                        autoCouponSavings: autoSavings,
                        effectivePrice: effectivePrice,
                        price: {
                            finalPayablePrice: effectivePrice,
                            menuPrice: finalPrice,
                            basePrice: basePrice,
                            discount: autoSavings + (basePrice - finalPrice)
                        },
                        offerText: discountHeader + (discountSubHeader ? ` | ${discountSubHeader}` : ''),
                        couponCode: couponCode,
                        couponDescription: allDiscountText,
                        ...
                        metadata: {
                            ...
                            cloudinaryImageId: info.imageId,
                            restaurantImageId: restInfo?.cloudinaryImageId,
                            isVeg: info.isVeg === 1,
                            itemBadge: info.badgesV2?.badges?.[0]?.description,
        ...
        console.error('[LiveSearch] Error fetching Swiggy dishes:', error);
        return [];
    }
}
